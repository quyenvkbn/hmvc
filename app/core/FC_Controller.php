...
class FC_Controller extends MX_Controller{
	...
	function __construct(){
		...
		$frontend_total_visited = $this->session->userdata('frontend_total_visited');
		if(!isset($frontend_total_visited) || empty($frontend_total_visited)){
			$this->session->set_userdata('frontend_total_visited', 'ok');
			$temp_1 = array(
				'ip' =>  $_SERVER['REMOTE_ADDR'],
				'url' => $_SERVER['PHP_SELF'],
				'created' => gmdate('Y-m-d H:i:s', time() + 7*3600),
			);
			if(isset($temp_1) && is_array($temp_1) && count($temp_1)){
				$this->db->insert('users_online', $temp_1);
			}
		}
