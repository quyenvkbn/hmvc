...
class Articles extends FC_Controller{
	...
	public function Create(){
		...
		if($this->input->post('create')){
			...
			$data['hedaotaoid'] =  $this->input->post('hedaotao');
			$data['avatar'] = $this->input->post('images');
			$data['avatar2'] = $this->input->post('images2');
			...
			if ($this->form_validation->run($this)){
				...
				$resultid = $this->BackendArticles_Model->Create($this->fcUser, $data['catalogue'], $album_data, $this->fclang);
				if($resultid > 0){
	...
	public function Update($id = 0){
		...
		$data['countryid'] = $this->BackendArticles_Model->ReadByModule($id, 'country');
		$data['hedaotaoid'] = $this->BackendArticles_Model->ReadByModule($id, 'hedaotao');
		...
		if($this->input->post('update')){
			...
			if ($this->form_validation->run($this)){
				...
				$flag = $this->BackendArticles_Model->UpdateByPost('id', $id, $this->fcUser, $catalogues, $album_data);
				if($flag > 0){
