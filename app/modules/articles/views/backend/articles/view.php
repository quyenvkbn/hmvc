<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li class="active"><a href="<?php echo site_url('articles/backend/articles/view');?>">Bài viết</a></li>
	</ol>
</section>
		...
		<div class="box-header">
			<h3 class="box-title pull-right">
				<div class="btn-group add-sort">
					...
					<a href="<?php echo site_url('articles/backend/articles/create');?>" class="btn btn-add  btn-flat"><i class="fa fa-plus"></i> Thêm bài viết</a>
				</div>
			</h3>
			<div class="box-tools pull-left">
				<span class="gcheckAction pull-left btn btn-default" data-module="articles"><i class="fa fa-trash"></i></span>
				<form class="pull-left" method="get" action="<?php echo site_url('articles/backend/articles/view');?>">
		...
		<div class="box-body table-responsive no-padding">
			...
			<table class="table" id="diagnosis-list">
				...
				<?php 
					...
					$_catalogue_list = $this->BackendArticlesCatalogues_Model->_get_where(array(
					...
					$href = rewrite_url($item['canonical'], $item['slug'], $item['id'], 'articles');
				...
				<tr>
					...
					<td style="width:650px;">
						<article class="article-view-1">
							...
							<div class="col-sm-10">
								...
								<div class="meta">
									<span class="user-create"><i class="fa fa-user"></i> <a href="<?php echo site_url('articles/backend/articles/view?userid='.$item['userid_created'].'') ?>" title=""><?php echo $item['fullname']; ?></a></span>
								...
								<span class="user-create"><i class="fa fa-user"></i> <a href="<?php echo site_url('articles/backend/articles/view?userid='.$item['userid_updated'].'') ?>" title=""><?php echo $item['fullname_update']; ?></a></span>
								 <?php } ?>
								<?php foreach($_catalogue_list as $keyCat => $valCat){ ?>
								<span><i class="fa fa-folder-open"></i> <a href="<?php echo site_url('articles/backend/articles/view?cataloguesid='.$valCat['id'].''); ?>" title=""><?php echo $valCat['title']; ?></a></span>
								<?php }} ?>
								</div>
							</div>
						</article>
					...
					<td>
						<a href="<?php echo site_url('articles/backend/articles/set/publish/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
							<img src="<?php echo ($item['publish'] > 0)? 'templates/backend/images/publish-check.png':'templates/backend/images/publish-deny.png'; ?>" alt="" />
						</a>
					</td>
					<td>
						<a href="<?php echo site_url('articles/backend/articles/set/highlight/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
					...
					<td>
						<a href="<?php echo site_url('articles/backend/articles/set/ishome/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
							<img src="<?php echo ($item['ishome'] > 0)? 'templates/backend/images/publish-check.png':'templates/backend/images/publish-deny.png'; ?>" alt="" />
						</a>
					</td>
					<td>
						<a href="<?php echo site_url('articles/backend/articles/set/isfooter/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
							<img src="<?php echo ($item['isfooter'] > 0)? 'templates/backend/images/publish-check.png':'templates/backend/images/publish-deny.png'; ?>" alt="" />
						</a>
					</td>
					<td class="text-right">
						<div class="btn-group">
							<a href="<?php echo site_url('articles/backend/articles/delete/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default"><span class="fa fa-trash"></span></a>
							<a href="<?php echo site_url('articles/backend/articles/update/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default"><i class="fa fa-edit"></i></a>
							<a href="<?php echo site_url('articles/backend/articles/read/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default"><i class="fa fa-eye"></i></a>
						</div>
					 </td>
				</tr>
				<?php } ?>
