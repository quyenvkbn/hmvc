...
class Comments extends FC_Controller{
	...
	public function listComment(){
		...
		if(isset($data['listComment']) && is_array($data['listComment']) && count($data['listComment'])){
			$html = $html .'<li class="total-cmt uk-clearfix">';
				$html = $html .'<span class="fll"> Có '.$config['total_rows'].' bình luận: </span>';
			...
			foreach($data['listComment'] as $key => $val){
			 ...
			 //    $name_alias = 'A';
				...
				$html = $html . '<li class="item-comments mb10">';
					$html = $html .'<div class="info uk-flex mb5 lib-grid-10">';
						if ($val['type'] == 0) {
							$html = $html .'<div class="avatar ec-cover">';
								$html = $html .'<img src="templates/frontend/resources/img/no-image.jpg" alt="Avatar" />';
							$html = $html .'</div>';
						}
						$html = $html.'<div class="author">';
							$html = $html .'<div class="meta mb10"><span class="user uk-text-bold">'.$val['fullname'].'</span> </div>';
							$html = $html .'<div class="meta"><span class="date">'.$val['created'].' </span> </div>';
						$html = $html.'</div>';
					$html = $html .'</div>';
					...
					$html = $html.'<div class="amenu uk-flex uk-flex-middle lib-grid-20">';
						$html = $html.'<span class="item-reply" data-id="'.$val['id'].'" data-type="1"><i class="fa fa-reply"></i> Trả lời</span>';
					$html = $html.'</div>';
					$html = $html.'<div class="reply-comment"></div>';
					if(isset($val['child']) && is_array($val['child']) && count($val['child'])){
						$string = list_sub_comments($val['child']);
						$html = $html.$string;
					}
				$html = $html . '</li>';
			}
