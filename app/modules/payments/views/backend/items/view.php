...
<section class="content" id="print">
  <div class="row">
	  ...
	  <div class="box">
		...
		<div class="box-body table-responsive no-padding">
			...
			<table class="table table-hover" id="diagnosis-list">
				...
				<tr>
				<?php 
					$href = rewrite_url($item['detail']['canonical'], $item['detail']['slug'], $item['detail']['id'], 'products');
				...
				?>
					...
					<td style="text-align:right;"><?php echo $item['qty'] ?> x <?php echo str_replace(',','.',number_format($item['price'])); ?></td>
					<td style="text-align:right;width:80px;"><strong><?php echo str_replace(',','.',number_format($item['subtotal'])); ?></strong></td>
				</tr>
				<?php } ?>
				<tr>
					<td colspan="4"><strong>Tổng</strong></td>
					<td style="width:40px;"><span class="<?php echo ($payment['status'] == 'success') ? 'done' : 'not-done'; ?>" style="font-size:15px;font-weight:bold;"><?php echo str_replace(',','.',number_format($payment['total_price'])); ?>đ</span></td>
				</tr>
			</table>
			</form>
		</div><!-- /.box-body -->
	...
	<div class="col-xs-3">
		<div class="box">
			...
			<div class="box-body">
				...
				<?php $district = location_dropdown('Thành phố', array('id' => $payment['districtid']));?>
	...
	<!-- 			<p><span style="font-weight:bold;">Tỉnh / Thành phố: </span> <span style="font-weight:bold;margin-left:10px;"><?php echo $city[$payment['cityid']]; ?></span></p>
				<p><span style="font-weight:bold;">Quận / Huyện: </span> <span style="font-weight:bold;margin-left:10px;"><?php echo $district[$payment['districtid']]; ?></span></p> -->
				...
				<?php $map = $payment['address'].' '.$district[$payment['districtid']].' '.$city[$payment['cityid']];  ?>
		...
		<div class="modal-content">
			...
			<div class="modal-body" style="width: 100%; float: left;">
				...
				<!-- <div class="col-xs-6 item">
					...
					<?php echo form_dropdown('cityid', location_dropdown('Thành phố', array('parentid' => 0)), set_value('cityid', ''), 'style="width: 100%;" id="cityid"');?>
				</div>
				<div class="col-xs-6 item">
