...
class Cart extends FC_Controller{
	...
	public function addpayment(){
		...
		$this->form_validation->set_rules('phone', 'Số điện thoại', 'trim|required|is_numeric');
		if ($this->form_validation->run($this)){
			...
			$Products = $this->FrontendProducts_Model->ReadByField('id', $data['prdid'], $this->fc_lang );
			$arr = array(
				'0' => array(
					'id' => $data['prdid'],
					'name' => $Products['title'],
			...
			$_paymentid = $this->Autoload_Model->_create(array(
				...
				'data' => array(
					...
					'publish' => 1,
					'status' => 'wait',
					'userid' => $Products['userid_created'],
					'send' => 0,
	...
	public function addtocart(){
        ...
        $html = $html . '<li class="item prices text-right">'.$this->lang->line('cart_products_price').'</li>';
        ...
        foreach($cart as $key => $val){
            ...
            $html = $html . '</div>';
            $html = $html . '</li>';
            $html = $html . '<li class="prices text-right">';
