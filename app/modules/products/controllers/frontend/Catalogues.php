	...
	public function View($id = 0, $page = 1){
		...
		if($config['total_rows'] > 0){
			...
			$data['productsList'] = $this->FrontendProducts_Model->_view(array(
				'select' => '`pr`.`id`, `pr`.`title`, `pr`.`slug`, `pr`.`canonical`, `pr`.`images`, `pr`.`description`, `pr`.`price`, `pr`.`saleoff`, `pr`.`banner`, `pr`.`highlight`, `pr`.`psale`, `pr`.`isfooter`,`pr`.`created`',
				'modules' => 'products',
				'order_by' => '`pr`.`order` asc ,`pr`.`id` desc',
		...
		$data['url'] = rewrite_url($DetailCatalogues['canonical'], $DetailCatalogues['slug'], $DetailCatalogues['id'], 'products');
