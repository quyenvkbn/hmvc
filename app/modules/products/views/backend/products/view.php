		...
		<div class="box-header">
			<h3 class="box-title pull-right">
				<div class="btn-group add-sort">
					...
					<a href="<?php echo site_url('products/backend/products/create');?>" class="btn btn-add  btn-flat"><i class="fa fa-plus"></i> Thêm Sản phẩm</a>
				</div>
			</h3>
			<div class="box-tools pull-left">
				<span class="gcheckAction pull-left btn btn-default" data-module="products"><i class="fa fa-trash"></i></span>
				<form class="pull-left" method="get" action="<?php echo site_url('products/backend/products/view');?>">
		...
		<div class="box-body table-responsive no-padding">
			...
			<table class="table" id="diagnosis-list">
				...
				<tr>
					...
					<td style="width:650px;">
						<article class="article-view-1">
							...
							<div class="col-sm-10">
								...
								<div class="meta">
									<span class="user-create"><i class="fa fa-user"></i> <a href="<?php echo site_url('products/backend/products/view?userid='.$item['userid_created'].'') ?>" title=""><?php echo $item['fullname']; ?></a></span>
								...
								<span class="user-create"><i class="fa fa-user"></i> <a href="<?php echo site_url('products/backend/products/view?userid='.$item['userid_updated'].'') ?>" title=""><?php echo $item['fullname_update']; ?></a></span>
								 <?php } ?>
								<?php foreach($_catalogue_list as $keyCat => $valCat){ ?>
								<span><i class="fa fa-folder-open"></i> <a href="<?php echo site_url('products/backend/products/view?cataloguesid='.$valCat['id'].''); ?>" title=""><?php echo $valCat['title']; ?></a></span>
								<?php }} ?>
								</div>
							</div>
						</article>
					...
					<td>
						<a href="<?php echo site_url('products/backend/products/set/publish/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
							<img src="<?php echo ($item['publish'] > 0)? 'templates/backend/images/publish-check.png':'templates/backend/images/publish-deny.png'; ?>" alt="" />
						</a>
					</td>
					<td>
						<a href="<?php echo site_url('products/backend/products/set/ishome/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
							<img src="<?php echo ($item['ishome'] > 0)? 'templates/backend/images/publish-check.png':'templates/backend/images/publish-deny.png'; ?>" alt="" />
						</a>
					</td>
					<td>
						<a href="<?php echo site_url('products/backend/products/set/highlight/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
					...
					<td class="text-right">
						<div class="btn-group">
							<a href="<?php echo site_url('products/backend/products/delete/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default"><span class="fa fa-trash"></span></a>
							<a href="<?php echo site_url('products/backend/products/update/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default"><i class="fa fa-edit"></i></a>
							<a href="<?php echo site_url('products/backend/products/read/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default"><i class="fa fa-eye"></i></a>
						</div>
					 </td>
				</tr>
				<?php } ?>
