...
class Catalogues extends FC_Controller{
	...
	public function View($id = 0, $page = 1){
		...
		$page = (int)$page;
		$seoPage = '';
		$DetailCatalogues = $this->FrontendVideosCatalogues_Model->ReadByField('id', $id);
		...
		$config['total_rows'] = $this->FrontendVideos_Model->_count(array(
			'select' => '`pr`.`id`',
			'modules' => 'videos',
		), $DetailCatalogues, $this->fc_lang);
		
		
		$config['base_url'] = rewrite_url($DetailCatalogues['canonical'], $DetailCatalogues['slug'], $DetailCatalogues['id'], 'videos_catalogues', FALSE, TRUE);
		if($config['total_rows'] > 0){
			...
			$config['full_tag_open'] = '<div class="pagenavi wow fadeInUp"><ul><li>';
			...
			$page = $page - 1;
			$data['videosList'] = $this->FrontendVideos_Model->_view(array(
				'select' => '`pr`.`id`, `pr`.`title`, `pr`.`slug`, `pr`.`canonical`, `pr`.`images`, `pr`.`videos_code`, `pr`.`description`, `pr`.`viewed`',
				'modules' => 'videos',
		...
		$data['highlight_post'] = $this->FrontendVideos_Model->_read_condition(array(
		...
		$data['template'] = 'videos/frontend/catalogues/view';
